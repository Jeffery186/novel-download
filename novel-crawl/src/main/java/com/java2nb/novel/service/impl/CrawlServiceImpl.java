...
public class CrawlServiceImpl implements CrawlService {
    ...
    private final BookCrawlSourceMapper bookCrawlSourceMapper;
    ...
    @Override
    public boolean parseBookAndSave(int catId, RuleBean ruleBean, Integer sourceId, String sourceBookId) {
        Long bookId ;
        Book book = CrawlParser.parseBook(ruleBean, sourceBookId);
...
*/
        ...
        //判断小说爬虫源关联表是否有该记录，没有则插入
        long count = bookCrawlSourceMapper.count(
                countFrom(BookCrawlSourceDynamicSqlSupport.bookCrawlSource)
                        .where(BookCrawlSourceDynamicSqlSupport.bookId, isEqualTo(bookId))
                        .and(BookCrawlSourceDynamicSqlSupport.sourceId, isEqualTo(sourceId))
                        .build().render(RenderingStrategies.MYBATIS3));

        if (count == 0) {
            BookCrawlSource bookCrawlSource = new BookCrawlSource();
            bookCrawlSource.setBookId(bookId);
            bookCrawlSource.setSourceId(sourceId);
            bookCrawlSource.setSourceBookId(sourceBookId);
            bookCrawlSource.setCatId(catId);
            bookCrawlSource.setCreateTime(new Date());

            bookCrawlSourceMapper.insertSelective(bookCrawlSource);
